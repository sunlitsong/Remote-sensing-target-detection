...
class FSVODDataset(Dataset):
    ...
    def _prepare_episode(self, idx):
        ...
        for frame_data in episode['query_video']['frames']:
            ...
            # Prepare query bboxes for this frame (in pixel coordinates on original image)
            query_bboxes = []
            objects = []
            for obj in frame_data['objects']:
                if obj['category_id'] in episode['classes']:
                    # bbox is in [x, y, w, h] format (pixel coordinates on original image)
                    x, y, w, h = obj['bbox']
                    cx, cy = x + w / 2, y + h / 2
                    objects.append({
                        'category_id': obj['category_id'],
                        'bbox': [cx, cy, w, h],  # [cx, cy, w, h] in target_size (for training loss)
                    })
